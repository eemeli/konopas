...
function Stars(e,t){t=t||{},this.name="konopas."+e+".stars",this.store=t.store||localStorage||sessionStorage||new function(){var e={};this.getItem=
function(t){return e[t]},this.setItem=
function(t,n){e[t]=n}},this.tag=t.tag||"has_star",this.server=!1,this.data=this.read()}
...
function pretty_date(e){var t={weekday:"long",month:"long",day:"numeric"},n=e instanceof Date?e:parse_date(e);if(!n)return e;Math.abs(n-Date.now())>5184e6&&(t.year="numeric");var r=n.toLocaleDateString(ko.lc,t);return r.charAt(0).toUpperCase()+r.slice(1)}
...
function(e){var n="tags",r=_prog_hash({tag:e}),i=e.split(":");i.length>1&&(n=i.shift(),e=i.join(":"));var a='<a href="'+r+'">'+e+"</a>";t[n]?t[n].push(a):t[n]=[a]});var n=[];for(var r in t)n.push(i18n_txt("item_tags",{T:r})+": "+t[r].join(", "));return'<div class="discreet">'+n.join("<br>")+"</div>\n"}
...
function item_show_list(e,t){var n=document.createDocumentFragment(),r="",i="";e.length>(t?1:0)&&e.length<ko.expand_all_max_items&&(n.appendChild(_new_elem("div","item_expander","» ")).appendChild(_new_elem("a","js-link",i18n_txt("Expand all"))).id="item_expander_link");for(var a=0,o=e.length;o>a;++a){if(e[a].date!=r){if(e[a].date<r)return void item_show_list(e.sort(_item_sort),t);r=e[a].date,i="",n.appendChild(_new_elem("div","new_day",pretty_date(e[a].date)))}if(e[a].time!=i){if(e[a].time<i)return void item_show_list(e.sort(_item_sort),t);i=e[a].time,n.appendChild(document.createElement("hr")),n.appendChild(_new_elem("div","new_time",pretty_time(e[a].time))).setAttribute("data-day",i18n_txt("weekday_short_n",{N:parse_date(e[a].date).getDay()}))}n.appendChild(_item_el(e[a]))}for(var s=EL("prog_ls");s.firstChild;)s.removeChild(s.firstChild);s.appendChild(n),server&&server.decorate_list(s);var l=EL("item_expander_link");l&&(l.onclick=
function(){var e=s.getElementsByClassName("item"),t=i18n_txt("Expand all");if(l.textContent==t){for(var n=0,r=e.length;r>n;++n)e[n].parentNode.classList.add("expanded"),_item_show_extra(e[n],e[n].id.substr(1));l.textContent=i18n_txt("Collapse all")}else{for(var n=0,r=e.length;r>n;++n)e[n].parentNode.classList.remove("expanded");l.textContent=t}});for(var c=s.getElementsByClassName("item_star"),a=0,o=c.length;o>a;++a)c[a].onclick=
function(){return stars.toggle(this,this.id.substr(1)),!1};for(var d=stars.list(),a=0,o=d.length;o>a;++a){var _=EL("s"+d[a]);_&&_.classList.add("has_star")}if(t){var p=document.getElementById("p"+t);p&&(p.parentNode.classList.add("expanded"),_item_show_extra(p,t),e.length>1&&p.scrollIntoView())}}
...
function update_next_select(e){var t=EL("next_time_select");if(t){var n=30,r=[-12*n,12*n],i=new Date;i.setMinutes(15*Math.floor(i.getMinutes()/15)+r[0]),t.options.length=0;for(var a=r[0];a<=r[1];a+=n){var o=document.createElement("option");o.value=a,o.text=i18n_txt("weekday_n",{N:i.getDay()})+", "+pretty_time(i),a==e&&(o.selected=!0),a||(o.id="next_time_select_now"),t.add(o),i.setMinutes(i.getMinutes()+n)}t.onchange=
...
function update_next_list(e){var t=EL("next_time_select"),n=t.selectedIndex>=0?parseInt(t[t.selectedIndex].value):0;n||(n=0),update_next_select(n);var r=new Date;r.setMinutes(15*Math.floor(r.getMinutes()/15)+n);var i=string_date(r),a=string_time(r);r.setMinutes(r.getMinutes()-r.getTimezoneOffset());for(var o={},s=0,l=r.valueOf()+36e5,c=0,d=0,_=program.length;_>d;++d){var p=program[d];if(!(p.date<i||p.date==i&&p.time<a)){var u=Date.parse(p.date+"T"+p.time);if((!s||s>u)&&u>l&&(s=u),"next_by_room"==e){if(p.loc){if(o[p.loc[0]]){if(o[p.loc[0]].date<p.date)continue;if(o[p.loc[0]].date==p.date&&o[p.loc[0]].time<p.loc[0])continue}o[p.loc[0]]=p}}else l>=u&&(o["n"+ ++c]=p)}}var f=[];for(var h in o)f.push(o[h]);if(f.length>0)EL("next_start_note").textContent="";else if(s){var m=new Date;m.setMinutes(m.getMinutes()+n-m.getTimezoneOffset());var v=Math.floor((s-m)/6e4),g=Math.floor(v/60);g>=1&&(v-=60*g),EL("next_start_note").textContent=i18n_txt("next_start",{H:g,M:v})}else EL("next_start_note").textContent=i18n_txt("next_ended");item_show_list(f)}
...
function(e){return e.id});r.sort();var i=r.length,a=stars.persistent()?"":"<p>"+i18n_txt("star_no_memory",{SERVER:!!server}),o=stars.list();o.sort();var s=o.length;if(s||i){var l="#star/set:"+o.join(",");if(i)if(arrays_equal(r,o))a+="<p>"+i18n_txt("star_export_this",{THIS:l})+"<p>"+i18n_txt("star_export_share",{SHORT:link_to_short_url(location.href),QR:link_to_qr_code(location.href)}),server&&server.show_ical_link(t);else{var c=array_overlap(r,o),d=i-c,_=n.length-i;if(a+="<p>"+i18n_txt("star_import_this",{THIS:location.href})+"<p>"+i18n_txt("star_import_diff",{PREV:s,NEW:d,SAME:c}),_&&(a+=" "+i18n_txt("star_import_bad",{BAD:_})),s&&c==s||(a+='<p>&raquo; <a href="#star" id="star_set_set">'+i18n_txt("star_set")+"</a>"),s){if(c!=s){var p=[];d&&p.push(i18n_txt("add_n",{N:d})),p.push(i18n_txt("discard_n",{N:s-c})),a+=" ("+p.join(", ")+")"}d&&(a+='<p>&raquo; <a href="#star" id="star_set_add">'+i18n_txt("star_add",{N:d})+"</a>")}var u=EL("star_set_set");u&&(u.onclick=
function(){return stars.set(r),!0});var f=EL("star_set_add");f&&(f.onclick=
function(){return stars.add(r),!0})}else a+='<p id="star_links">&raquo; '+i18n_txt("star_export_link",{URL:l,N:s});var h=program.filter(
function(e){return o.indexOf(e.id)>=0||r.indexOf(e.id)>=0});if(item_show_list(h),i){document.body.classList.add("show_set");for(var m=0;i>m;++m){var v=EL("s"+r[m]);v&&v.classList.add("in_set")}}}else a+="<p>"+i18n_txt("star_hint"),EL("prog_ls").innerHTML="";t.innerHTML=a}
...
function _prog_set_location_id(e){var t=_prog_get_filters(!0);return e&&!t.day&&(t.day=ko.show_all_days_by_default?"all_days":_prog_default_day()),t.id=e,_prog_set_filters(t,!0)}
...
function(){try{(new Date).toLocaleDateString("i")}catch(e){return"RangeError"!==e.name}return!0}()&&(Date.prototype._orig_toLocaleDateString=Date.prototype.toLocaleDateString,Date.prototype.toLocaleDateString=
function(e,n){if(!arguments.length||"undefined"==typeof i18n)return this._orig_toLocaleDateString();var r=
function(t,n){return t in i18n[e]?i18n[e][t](n):t},i=r("weekday_n",{N:this.getDay()}),a=this.getDate(),o=r("month_n",{N:this.getMonth()}),s=i+", "+a+" "+o;return n&&n.year&&(s+=" "+t.getFullYear()),s}),"undefined"==typeof document||"classList"in document.createElement("a")||!
...
function(e){return i18n.p(e,"N",0,"en",{0:"Sunday",1:"Monday",2:"Tuesday",3:"Wednesday",4:"Thursday",5:"Friday",6:"Saturday",other:"???"})},weekday_short_n:
function(e){return i18n.p(e,"N",0,"en",{0:"Sun",1:"Mon",2:"Tue",3:"Wed",4:"Thu",5:"Fri",6:"Sat",other:"???"})},month_n:
...
function(e){return"Program id <b>"+i18n.v(e,"ID")+"</b> not found!"},item_tags:
function(e){return i18n.s(e,"T",{tags:"Tags",track:"Tracks",type:"Types",other:i18n.v(e,"T")})},next_ended:
...
function(){return'To "star" a program item, click on the square next to it. Your selections will be remembered, and shown in this view. You currently don\'t have any program items selected, so this list is empty.'},star_no_memory:
function(e){return'For some reason, your <a href="http://en.wikipedia.org/wiki/Web_storage">localStorage</a> is not enabled, so your selection will not be remembered between sessions. '+i18n.s(e,"SERVER",{"true":"Please login to enable persistent storage.",other:""})},filter_sum_id:
...
function(){return"Once you've verified your e-mail address, you'll be able to sync your data between different clients (including external calendars), as well as vote & comment on items. After signing in, no information will be transmitted to these external services."
}}}(this),Server.prototype.disconnect=
...
function(){return JSON.parse(this.store&&this.store.getItem(this.name)||"{}")},Stars.prototype.write=
function(){try{this.store&&this.store.setItem(this.name,JSON.stringify(this.data))}catch(e){if(e.code!=DOMException.QUOTA_EXCEEDED_ERR||0!=this.store.length)throw e}},Stars.prototype.persistent=
function(){return this.store&&(this.store==localStorage||this.server&&this.server.connected)},Stars.prototype.list=
