...
function prog_filter_change(ev) {
	...
	var filters = prog_get_filters();
	filters[key] = value;
	if (prog_set_filters(filters)) ev.stopPropagation();
}
...
if (sf) {
	...
	sf.onreset = function() { prog_set_filters({}); };
}
...
// init item expansion
EL('prog_ls').onclick = function(ev) {
	ev = ev || window.event;
